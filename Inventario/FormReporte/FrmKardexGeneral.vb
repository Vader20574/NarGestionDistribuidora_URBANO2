...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FrmKardexGeneral))
...
RegEquiv:
    ...
    Sub Procesar_Saldo(ByVal fechaCierre As String)
        ...
        Else
            ...
            TmpInsertDatos("KARDEX_GENERAL_REPORT_FALTANTES_INGSAL '" & codempresa & "','" & Fecinicial & "','" & Fecfinal & "','" & CboAlmacen.SelectedValue.ToString & "','" & CadenaCateg & "'," & Todo_Cate & ",'" & fechaCierre & "','MOVIDET',0,'" & rand & "'")
    ...
    Private Sub INSERTCIERREALDIA(ByVal FechaCierre As Date)
        If Todo_Cate = 1 Then
            TmpInsertDatos("INSERT INTO TMP_KARDEX (CORREL,CODEMPRESA,CDALMA, CDG01, CDARTI, STOCKINI, STOCKREC, COMPRAS, INGAJUSTE, INGTRANSF, INGOTROS,VENTAS, SALAJUSTE, SALTRANSF, SALOTROS )  " &
                          ...
                          "FROM CIERRE_MES_INVENTARIO,ARTICULO WHERE CIERRE_MES_INVENTARIO.ARTICULO=ARTICULO.CODARTI AND ARTICULO.FORMU = 0 AND CIERRE_MES_INVENTARIO.ALMACEN ='" & Trim(CboAlmacen.SelectedValue) & "'  AND FECHA='" & FechaCierre & "'")
        Else
            TmpInsertDatos(" INSERT INTO TMP_KARDEX (CORREL,CODEMPRESA,CDALMA, CDG01, CDARTI, STOCKINI, STOCKREC, COMPRAS, INGAJUSTE, INGTRANSF, INGOTROS,VENTAS, SALAJUSTE, SALTRANSF, SALOTROS ) " &
                             " SELECT '" & rand & "','" & codempresa & "',ALMACEN,ARTICULO.CODLINEA,CIERRE_MES_INVENTARIO.ARTICULO,CIERRE_MES_INVENTARIO.STOCK AS STKINICIO,0,0,0,0,0,0,0,0,0 " &
                             " FROM CIERRE_MES_INVENTARIO,ARTICULO WHERE  ARTICULO.FORMU = 0  AND  CIERRE_MES_INVENTARIO.ARTICULO=ARTICULO.CODARTI AND ARTICULO.CODLINEA IN(" & CadenaCateg & ")  AND CIERRE_MES_INVENTARIO.ALMACEN ='" & Trim(CboAlmacen.SelectedValue) & "'  AND FECHA='" & Format(FechaCierre, "dd/MM/yyyy") & "'")
        End If
    End Sub
