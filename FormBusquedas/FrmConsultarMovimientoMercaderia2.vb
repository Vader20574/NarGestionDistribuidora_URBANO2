Public Class FrmConsultarMovimientoMercaderia2
    ...
    Sub llenar()
        ...
        For Each dr As DataRow In Table_Filtro.Rows
            ...
            If rbNroMovimiento.Checked = True And TxtNroMovi.Text.Trim.Length <> 0 Then
                TABLE_MOV = TmpListarDatos("NSP_LISTAR_MOVIMIENTO_MERC_FILTRO  '" & codempresa & "','" & INOUT & "','" & 0 & "','" & TxtNroMovi.Text & "','" & dr.Item(0).ToString & "'," & 1)
                LLenar_Lista(TABLE_MOV, dr.Item(0).ToString, Movidet)
            End If

            If rbNroDocumento.Checked = True And TxtNroMovi.Text.Trim.Length <> 0 Then
                TABLE_MOV = TmpListarDatos("NSP_LISTAR_MOVIMIENTO_MERC_FILTRO  '" & codempresa & "','" & INOUT & "','" & 1 & "','" & TxtNroMovi.Text & "','" & dr.Item(0).ToString & "'," & 1)
                LLenar_Lista(TABLE_MOV, dr.Item(0).ToString, Movidet)
            End If
        Next

        If TxtNroMovi.Text = "" Then
            TABLE_MOV = TmpListarDatos("NSP_LISTAR_MOVIMIENTO_MERC  '" & codempresa & "','" & INOUT & "'")
            LLenar_Lista(TABLE_MOV)
        End If

        If rbNroMovimiento.Checked = True And TxtNroMovi.Text.Trim.Length <> 0 Then
            TABLE_MOV = TmpListarDatos("NSP_LISTAR_MOVIMIENTO_MERC_FILTRO  '" & codempresa & "','" & INOUT & "','" & 0 & "','" & TxtNroMovi.Text & "',''," & 0)
            LLenar_Lista(TABLE_MOV)
        End If
