...
Public Class FrmListarArticuloMercaderiaP
...
#Region "Variables"
    ...
    'Public Ingreso As New frm_produccion



    Public Ingreso As New frm_CartaFianza
...
#Region "Funciones"
    ...
    Private Sub CargarGridForm()
        ...
        TABLEART = TmpListarDatos("SP_LISTA_ARTICULO_MERC  '" & codempresa & "','" & CDZONA & "','" & 0 & "','" & CDALMACEN & "'")
...
#End Region
    ...
    Private Sub Devuelve()
        Try
            ...
            Ingreso.tarticulo.Text = Trim(GridArt.Item(GridArt.CurrentRowIndex, 0))
            Ingreso.txtdesarticulo.Text = Trim(GridArt.Item(GridArt.CurrentRowIndex, 1))


            TABLEART = TmpListarDatos("SP_SELECT_COMI  '" & Trim(GridArt.Item(GridArt.CurrentRowIndex, 0)) & "'")
            'TABLEART = TmpListarDatos("SP_LISTA_ARTICULO_FACT_FILTRO ", codempresa, CDZONA, txtdato.Text.Trim, txtdato.Text.Trim, CboFiltrar.SelectedIndex)
            If TABLEART.Rows.Count > 0 Then
                Ingreso.LblCostAnt.Text = TABLEART.Rows(0)(0).ToString()
            End If


            Dim TABLEPES As DataTable
            TABLEPES = TmpListarDatos("SP_SELECT_PESO  '" & Trim(GridArt.Item(GridArt.CurrentRowIndex, 0)) & "'")
            'TABLEART = TmpListarDatos("SP_LISTA_ARTICULO_FACT_FILTRO ", codempresa, CDZONA, txtdato.Text.Trim, txtdato.Text.Trim, CboFiltrar.SelectedIndex)
            If TABLEPES.Rows.Count > 0 Then
                Ingreso.txtPesoStandarP.Text = TABLEPES.Rows(0)(0).ToString()
            End If
    ...
    Private Sub txtdato_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtdato.TextChanged
        ...
        Else
            If rbcodigo.Checked = True Then
                Tabl_temp = CAyuda.ListarDatos("SP_LISTA_ARTICULO_MERC_FILTRO", codempresa, CDZONA, 0, txtdato.Text.Trim, CDALMACEN).Tables(0)
                GridArt.DataSource = Nothing
                GridArt.DataSource = Tabl_temp
            End If

            If rbdescripcion.Checked = True Then
                Tabl_temp = CAyuda.ListarDatos("SP_LISTA_ARTICULO_MERC_FILTRO", codempresa, CDZONA, 1, txtdato.Text.Trim, CDALMACEN).Tables(0)
                GridArt.DataSource = Nothing
                GridArt.DataSource = Tabl_temp
            End If
