Public Class FrmSalidaDeMercaderiaFP
...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.ClientSize = New System.Drawing.Size(1260, 755)
...
#End Region
    ...
    Private Sub BTNGUARDAR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNGUARDAR.Click
        ...
        If MODIFICAR = False Then
            ...
            If TmpInsertDatos("NSP_GRABAR_MOVIMIENTO_MERCADERIASP  '" & codempresa & "','" & tmovimiento.Text.Trim & "','" & cbAlmacen.SelectedValue.ToString.Trim & "','" & cbotipo.SelectedValue.ToString.Trim & "','" & IIf(cbProveedor.Enabled = True, cbProveedor.SelectedValue.ToString.Trim, "") & "','" & ctipodoc.SelectedValue.ToString.Trim & "','" & 0 & "','" & (CStr(CInt(TxtDoc1.Text)) + TxtDoc2.Text) & "' ,'" & Format(dtpfecha.Value, "dd/MM/yyyy") & "','" & CboMoneda.SelectedValue.ToString.Trim & "','" & LblSubtotal.Text & "','" & Format(CDbl(LblSubtotal.Text) / VCambioCompra, "#######0.00") & "','" & VCambioCompra & "','" & LblIGV.Text & "','" & Format(CDbl(LblIGV.Text) / VCambioCompra, "#######0.00") & "','" & LblTotal.Text & "','" & Format(CDbl(LblTotal.Text) / VCambioCompra, "#######0.00") & "', '" & TxtDesc.Text.Trim & "','" & Format((CDbl(LblValorVenta.Text) - CDbl(LblSubtotal.Text)), "#######0.00") & "','" & Format(((CDbl(LblValorVenta.Text) - CDbl(LblSubtotal.Text)) / VCambioCompra), "#######0.00") & "' ,'" & IIf(cbAlmacen.Enabled = True, cbAlmacen.SelectedValue.ToString.Trim, "") & "','" & CodUsuario & "','" & cboPeriodo.Text & "','" & txtEjercicio.Text.Trim & "','" & OBS & "','" & IIf(cbAlmacen.Enabled = True, cbAlmacen.SelectedValue.ToString.Trim, "") & "' ,'" & CRED & "','" & CboFormaPago.SelectedValue.ToString.Trim & "','" & NROPTOVTA & "','" & CDbl(txtFlete.Text) & "','" & CORREL & "' ,'" & IIf(TRANS = True, 1, 0) & "','" & CDbl(TxtPercep.Text) & "','" & CDbl(TxtDetrac.Text) & "','" & dtpfechaRecepcion.Text & "','" & 0 & "'") = True Then
                ...
                ' '' ''        If cbotipo.SelectedValue.ToString.Trim = "003" Then

                ' '' ''            If ctipodoc.SelectedValue.ToString.Trim = "07" Then

                ' '' ''                CAyuda.Ejecutar("Ins_COMPRA_X_PAGARxProc", codempresa, ctipodoc.SelectedValue, (CStr(CInt(TxtDoc1.Text)) + TxtDoc2.Text), CboMoneda.SelectedValue, cbProveedor.SelectedValue, -CDbl(LblTotal.Text), -CDbl(LblTotal.Text) / VCambioCompra, Format(dtpfecha.Value, "dd/MM/yyyy"), CboFormaPago.SelectedValue, "", CodUsuario, CDbl(TxtPercep.Text), "", "", "")
                ' '' ''            Else
                ' '' ''                CAyuda.Ejecutar("Ins_COMPRA_X_PAGARxProc", codempresa, ctipodoc.SelectedValue, (CStr(CInt(TxtDoc1.Text)) + TxtDoc2.Text), CboMoneda.SelectedValue, cbProveedor.SelectedValue, CDbl(LblTotal.Text), CDbl(LblTotal.Text) / VCambioCompra, Format(dtpfecha.Value, "dd/MM/yyyy"), CboFormaPago.SelectedValue, "", CodUsuario, CDbl(TxtPercep.Text), "", "", "")
        ...
        Else
            ...
            Else
                varprov = String.Empty
            End If

            If TmpInsertDatos("NSP_MODIFICA_MOVIMIENTO_MERCADERIASP  '" & codempresa & "','" & tmovimiento.Text.Trim & "','" & cbAlmacen.SelectedValue.ToString.Trim & "','" & cbotipo.SelectedValue.ToString.Trim & "','" & varprov & "','" & ctipodoc.SelectedValue.ToString.Trim & "','" & 0 & "','" & (CStr(CInt(TxtDoc1.Text)) + TxtDoc2.Text) & "' ,'" & Format(dtpfecha.Value, "dd/MM/yyyy") & "','" & CboMoneda.SelectedValue.ToString.Trim & "','" & LblSubtotal.Text & "','" & Format(CDbl(LblSubtotal.Text) / VCambioCompra, "#######0.00") & "','" & VCambioCompra & "','" & LblIGV.Text & "','" & Format(CDbl(LblIGV.Text) / VCambioCompra, "#######0.00") & "','" & LblTotal.Text & "','" & Format(CDbl(LblTotal.Text) / VCambioCompra, "#######0.00") & "', '" & TxtDesc.Text.Trim & "','" & Format((CDbl(LblValorVenta.Text) - CDbl(LblSubtotal.Text)), "#######0.00") & "','" & Format(((CDbl(LblValorVenta.Text) - CDbl(LblSubtotal.Text)) / VCambioCompra), "#######0.00") & "' ,'" & IIf(cbAlmacen.Enabled = True, cbAlmacen.SelectedValue.ToString.Trim, "") & "','" & CodUsuario & "','" & cboPeriodo.Text & "','" & txtEjercicio.Text.Trim & "','" & OBS & "','" & IIf(cbAlmacen.Enabled = True, cbAlmacen.SelectedValue.ToString.Trim, "") & "' ,'" & CRED & "','" & CboFormaPago.SelectedValue.ToString.Trim & "','" & NROPTOVTA & "','" & CDbl(txtFlete.Text) & "','" & CORREL & "' ,'" & IIf(TRANS = True, 1, 0) & "','" & CDbl(TxtPercep.Text) & "','" & CDbl(TxtDetrac.Text) & "','" & dtpfechaRecepcion.Text & "','" & MoviCab & "','" & MoviDet & "'") = True Then
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnBusMovi.Click
        ...
        Dim FRMBUSC As New FrmConsultarMovimientoMercaderiaSP
        FRMBUSC.INOUT = 0
        FRMBUSC.ShowDialog()
        ...
        TABLE_CAB_M = TmpListarDatos("NSP_TRAER_MOVIMIENTO_MERC_MSP  '" & codempresa & "','" & tmovimiento.Text & "','" & cbAlmacen.SelectedValue.ToString.Trim & "','" & IIf(IsNothing(cbotipo.SelectedValue), "", cbotipo.SelectedValue) & "','" & 0 & "','" & rand & "'")
        ...
        TABLE_CAB = TmpListarDatos("NSP_TRAER_MOVIMIENTO_MERCSP  '" & codempresa & "','" & tmovimiento.Text & "','" & cbAlmacen.SelectedValue.ToString.Trim & "','" & IIf(IsNothing(cbotipo.SelectedValue), "", cbotipo.SelectedValue) & "','" & 0 & "','" & CORREL & "','" & MoviCab & "','" & MoviDet & "'")
    ...
    Private Sub BTNELIMINAR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNELIMINAR.Click
        ...
        If TmpInsertDatos("NSP_ANULAR_INGRESO_MERCADERIASP  '" & tmovimiento.Text.Trim & "','" & cbotipo.SelectedValue.ToString.Trim & "','" & (CStr(CInt(TxtDoc1.Text)) & TxtDoc2.Text.Trim) & "','" & ctipodoc.SelectedValue.ToString.Trim & "','" & codempresa & "','" & MoviCab & "','" & MoviDet & "'") = True Then
