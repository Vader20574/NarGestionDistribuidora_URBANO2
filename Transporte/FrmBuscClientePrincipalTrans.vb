Public Class FrmBuscClientePrincipalTrans
...
#Region "Funciones"
    ...
    Private Sub CargarGridForm()
        TABLECLIENT = TmpListarDatos("SP_LISTAR_CLIENTE_TRANS_DEPMAEST  '" & codempresa & "','" & 0 & "'")
    ...
    Private Sub txtdato_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtdato.TextChanged
        ...
        Else
            ...
            If Me.rbcodigo.Checked = True Then
                TABLECLIENT = CAyuda.ListarDatos("SP_LISTAR_CLIENTE_TRANS_FILTRO", codempresa.ToString(), 0, txtdato.Text).Tables(0)
                GridCli.DataSource = TABLECLIENT
            End If

            If Me.rbdescripcion.Checked = True Then
                TABLECLIENT = CAyuda.ListarDatos("SP_LISTAR_CLIENTE_TRANS_FILTRO", codempresa.ToString(), 1, txtdato.Text).Tables(0)
                GridCli.DataSource = TABLECLIENT
            End If

            If Me.rbdireccion.Checked = True Then
                TABLECLIENT = CAyuda.ListarDatos("SP_LISTAR_CLIENTE_TRANS_FILTRO", codempresa.ToString(), 2, txtdato.Text).Tables(0)
                GridCli.DataSource = TABLECLIENT
            End If

            If Me.rbruc.Checked = True Then
                TABLECLIENT = CAyuda.ListarDatos("SP_LISTAR_CLIENTE_TRANS_FILTRO", codempresa.ToString(), 3, txtdato.Text).Tables(0)
                GridCli.DataSource = TABLECLIENT
            End If
        End If
